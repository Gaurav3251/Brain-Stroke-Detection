...
def _min_mask_pixels(filter_size: int) -> float:
    # SEG_MIN_MASK_PX is defined at the canonical segmentation resolution IMG_SEG.
    return SEG_MIN_MASK_PX * (filter_size / IMG_SEG) ** 2


def collect_samples(
    use_overlay: bool = False,
    stroke_only: bool | None = None,
    mask_filter_size: int = IMG_SEG,
) -> List[dict]:
            ...
            if overlay_dir and os.path.isdir(overlay_dir):
                ...
                if os.path.exists(cand):
                    if use_overlay and stroke_only:
                        test_mask = overlay_to_mask(cand, mask_filter_size)
                        if test_mask.sum() < _min_mask_pixels(mask_filter_size):
                            skipped_empty += 1
                            continue
                    mask_path = cand
...
def build_loaders(task: str = "classify", img_size: int = IMG_CLS, use_overlay: bool = False, stroke_only: bool | None = None):
    all_s = collect_samples(use_overlay=use_overlay, stroke_only=stroke_only, mask_filter_size=img_size)
