...
from ..core.config import PLOT_DIR, PRED_DIR, CLASSES, NUM_CLASSES
...
def save_pred_plot(fig, name):
    os.makedirs(PRED_DIR, exist_ok=True)
    path = os.path.join(PRED_DIR, f"{name}.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Pred] -> {path}")
...
def plot_confusion_matrix(cm, model_name, out_dir="plots"):
    ...
    if out_dir == "predictions":
        save_pred_plot(fig, f"{model_name}_confusion_matrix")
    else:
...
def plot_sample_preds(model, dataset, model_name, task="classify", n=9):
    ...
    for i, j in enumerate(idx):
        ...
        with torch.no_grad():
            model_in = img_t.unsqueeze(0).to(model_device)
            lg = model(model_in)
        ...
        axes[i].imshow(disp)
        axes[i].set_title(
            f"GT:{CLASSES[lbl_idx]} | P:{CLASSES[pred]}({conf:.2f})",
    ...
    plt.tight_layout()
    save_pred_plot(fig, f"{model_name}_sample_preds")


def plot_sample_preds_with_predictor(dataset, model_name, predictor, n=9):
    idx = np.random.choice(len(dataset), min(n, len(dataset)), replace=False)
    cols = 3
    rows = (len(idx) + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(12, 4 * rows))
