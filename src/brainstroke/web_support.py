...
from .core.config import CLASSES, INFER_DIR
...
REPORTS_DIR = INFER_DIR.parent.parent / "web_reports"
...
def _build_asset_paths(image_path: str) -> ReportAssets:
    ...
    assets = {
        "classification": INFER_DIR / f"inference_{stem}_classification.png",
        "explainability": INFER_DIR / f"inference_{stem}_explainability.png",
        "metrics": INFER_DIR / f"inference_{stem}_metrics.png",
    }
    return ReportAssets(
...
def generate_web_report(
    ...
    results = predict_single_image(image_path, models, model_choice=model_choice, save_outputs=True)
